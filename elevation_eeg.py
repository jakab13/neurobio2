...
# todo work around (note to myself): avoid shuffling 600 elements until
#  "np.min(np.diff(np.random.choice(range(1 * 600)), 200, replace=False))) < min_dist" is satisfied
for i in range(2):
...
### start loop over trial sequence ###
else:
